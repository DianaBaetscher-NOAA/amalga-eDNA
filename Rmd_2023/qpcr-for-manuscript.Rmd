...
merged_df_both_years %>%
  filter(transect == "perpendicular" &
           depth == 0) %>%
  ggplot(aes(x = distance_from_pens, y = log_dna_ul, color = year)) +
  geom_point() +
  facet_grid(rows = vars(tide)) +
  theme_bw() +
...
tmp %>%
  ggplot() +
  #geom_jitter(aes(x = x_dist, y = y_dist, color = log_quant), size = 1, alpha = 0.9, width = 110, height = 90) +
  geom_point(aes(x = x_dist+xJit, y = y_dist+yJit, color = log_quant), size = 1, alpha = 0.9, width = 110, height = 90) +
...
box_plot_df <- for_anova %>%
  mutate(log_dna_ul = ifelse(log_dna_ul < 0, 0, log_dna_ul))
  

d.boxp <- ggplot(box_plot_df, aes(x=factor(depth), y=log_dna_ul, fill=tide)) +
  geom_boxplot() +
  theme_minimal() +
...
ggsave("pdf_outputs/depth_detections_by_tide.png", width = 4, height = 3)
```
