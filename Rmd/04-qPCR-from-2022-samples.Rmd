...
```{r}
samplesheet <- read_csv("C:/Users/diana.baetscher/Downloads/pcod_samplesheet.csv", skip = 19)

meta <- read_xlsx("C:/Users/diana.baetscher/Downloads/20221117_PCodGTSeq_Metadata_ABLGSubset.xlsx") %>%
  select(ABLG, AlternateID_s_)


new_samplesheet <- samplesheet %>%
  left_join(., meta, by = c("Sample_ID" = "AlternateID_s_")) %>%
  mutate(Sample_ID = ABLG) %>%
  select(-ABLG)
  
```
```{r}
new_samplesheet %>%
  filter(Sample_Well == "H12")
```


```{r}
new_samplesheet %>%
  write_csv("C:/Users/diana.baetscher/Downloads/20221122_pcod_revised_samplesheet.csv")
