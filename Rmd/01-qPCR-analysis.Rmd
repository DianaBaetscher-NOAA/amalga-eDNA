...
meta <- read_xlsx("../data/extraction_database.xlsx", sheet = "metadata")
...
```{r quick-look-at-samples-for-inhibition-test}
sample_list <- qpcr %>%
  filter(Task == "UNKNOWN") %>%
  select(`Sample Name`) %>%
  unique()

reruns <- read_xlsx("~/Desktop/2023-10-20_AmalgaInhibitionTest2021samples.xlsx", sheet = "Results", skip = 33)
...
sample_list %>%
  anti_join(., reruns_s) %>%
  left_join(., meta, by = c("Sample Name" = "Extraction ID")) %>%
  filter(`Sample Label` != "Ext_blank") %>%
  select(`Sample Name`) %>%
  unique() %>%
  write_csv("csv_outputs/2021_samples_for_inhibition_test.csv")
...
clean_meta %>% 
  write_csv("csv_outputs/amalga_clean_metadata.csv")
```
...
tmp_df %>%
  ...
  mutate(cycle_threshold = round(CT, 2)) %>%
  group_by(`sample`, `Sample Code`) %>%
  ggplot(aes(x = distance, y = ct, color = `Sample Code`, shape = Sample_type))+
  geom_point() +
  theme_bw() +
  xlab("distance from chum pens")
